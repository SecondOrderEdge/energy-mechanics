...
(function(){
  ...
  // ───── editable constants ─────
  // Flip these when the situation changes. Status options:
  //   "OPEN"       — normal operations, no notable risk
  //   "WATCH"      — background risk elevated; strait operational (DEFAULT)
  //   "RESTRICTED" — selective transits, escort regimes, partial disruption
  //   "CLOSED"     — strait effectively shut to commercial transit
  //
  // EVENT_START set to a Date enables the duration counter ("Days since X").
  // Set to null to hide the counter — appropriate for steady-state monitoring.
  const STATUS      = "WATCH";
  const EVENT_START = null;   // e.g. new Date("2025-06-13T00:00:00Z") during an active event
  const STATUS_NOTE = "Strait open and operational. Elevated background risk monitored since the brief Jun 2025 Israel-Iran exchange; no current blockade.";
  ...
  // These reflect a normal-operations / WATCH state. During an active event,
  // update these and flip STATUS / EVENT_START above. All values here are
  // editorial estimates — see Disclaimer panel on the page.
  const STATS = {
    transits_now: 58,        // close to normal ~60/day baseline
    waiting:      8,         // typical queue near approach
    waiting_mix:  { tankers: 5, bulk: 1, other: 2 },
    war_label:    "ELEVATED",
    war_pct:      0.18,      // hull war premium — slightly above ~0.15% baseline
    war_mult:     "~1.2× normal",
    vlcc_ws:      62,        // Worldscale points · normal-ish range
    vlcc_chg:     "near pre-2025 baseline",
    dwt:          "10.1M",
    dwt_pct:      "~98% of normal",
  };
  ...
  // ───── vessel seizures (verifiable historical events only) ─────
  // side: "iran" = seized by Iran/IRGC; "usuk" = seized by US/UK/coalition
  // Update the date / status fields when something new occurs. Don't add
  // speculative entries — keep this list to events with primary-source
  // reporting (Lloyd's List, USNI News, official DOJ filings).
  const SEIZURES = [
    { date:"Apr 15, 2024", side:"iran", actor:"Iran (IRGC)", vessel:"MSC Aries",
      flag:"Portugal", desc:"Container vessel seized near strait by IRGC helicopter rappel team. Crew of 25 detained, mostly released later.", status:"released" },
    { date:"Jan 11, 2024", side:"iran", actor:"Iran",       vessel:"St. Nikolas",
      flag:"Marshall Islands", desc:"Crude tanker (former Suez Rajan) seized by Iranian forces in Gulf of Oman. Same vessel had previously been used in a US-led Iranian oil interception.", status:"released" },
    ...
    { date:"May 3, 2023",  side:"iran", actor:"Iran (IRGC)", vessel:"Niovi",
      flag:"Panama", desc:"Crude tanker seized in Strait of Hormuz; Iranian officials cited damage to an Iranian vessel as pretext.", status:"released" },
    ...
    { date:"Apr 30, 2023", side:"usuk", actor:"United States", vessel:"Suez Rajan",
      flag:"Marshall Islands", desc:"US Justice Department forfeiture action — vessel carrying ~1 mb of Iranian crude in transit was directed to Houston for offload.", status:"resolved" },
  ...
  const TIMELINE = [
    { date:"JUN 13, 2025", type:"esc",
      head:"Israel launches direct strikes on Iran",
      desc:"Israeli air campaign against Iranian nuclear infrastructure, military bases, and IRGC leadership. Brent spiked ~$8/bbl on closure-threat headlines." },
    { date:"JUN 23, 2025", type:"mil",
      head:"US strikes Iranian nuclear sites",
      desc:"US conducts strikes on Fordow, Natanz, and Isfahan. Iran responds with missile attack on Al Udeid air base in Qatar; advance warning prevents casualties. Hormuz remained operationally open throughout." },
    { date:"JUN 24, 2025", type:"dip",
      head:"Israel–Iran ceasefire",
      desc:"Ceasefire holds. Brent gives back most of the war premium within two weeks as strait transit resumes near-normal levels. No actual blockade or extended shutdown occurred." },
    ...
    { date:"JAN 2024",      type:"inc",
      ...
      desc:"Bab el-Mandeb tanker traffic falls ~70% as most operators reroute around Cape of Good Hope, adding 10–14 days transit. Indirectly raises Hormuz importance for Asian flows." },
    { date:"2023",          type:"dip",
    ...
    { date:"2015",          type:"dip",
      ...
      desc:"Iran–P5+1 deal lifts most economic sanctions in exchange for nuclear constraints. Iranian oil exports rebound to ~2.5 mmbpd before 2018 US withdrawal." },
  ...
  function tickDuration(){
    if (!EVENT_START) return;   // counter is hidden when no active event
    const now = new Date();
  ...
  function renderStatus(){
    ...
    const since = document.getElementById("status_since");
    const duration = document.querySelector(".hz-duration");

    // Color by status level — green for OPEN, orange for WATCH, red for the
    // disruption states.
    if (big) {
      if (STATUS === "OPEN")          big.style.color = "var(--emp)";
      else if (STATUS === "WATCH")    big.style.color = "var(--unemp)";
      else                            big.style.color = "var(--layoff)";
    }
    if (since) since.textContent = STATUS_NOTE;

    // Only show the duration counter if there's an active event anchor.
    // Otherwise hide it — a steady-state "watch" mode has no meaningful
    // ticker to display.
    if (duration) {
      duration.style.display = EVENT_START ? "" : "none";
    }
  }
